    ...
    if message.text in ['help', '/help']:
        def help(message):
            ...
            bot.send_message(message.chat.id, 'Данный бот умеет выполнять различные арифметические операции, а также может '
                                              ...
                                              'Чтобы узнать погоду просто пропишите или нажмите /weather', reply_markup=markup, parse_mode='html')
        help(message)
        ...
        if message.text not in ['start', 'help', '/start', '/help', 'все_атрибуты', 'weather', '/weather']:
